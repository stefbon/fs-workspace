...
static void receive_msg_channel_data_init(struct ssh_channel_s *channel, struct ssh_payload_s *payload)
{
    queue_ssh_payload_channel(channel, payload);
}

static void receive_msg_channel_data_down(struct ssh_channel_s *channel, struct ssh_payload_s *payload)
{
    /* do nothing */
    free(payload);
}
...
static void receive_msg_channel_data(struct ssh_session_s *session, struct ssh_payload_s *payload)
{
    ...
    */
	...
	if (len + pos == payload->len) {
		...
		(* channel->receive_msg_channel_data)(channel, payload);
		payload=NULL;
...
static void receive_msg_channel_eof(struct ssh_session_s *session, struct ssh_payload_s *payload)
{
	...
	logoutput("receive_msg_channel_eof: channel %i", local_channel);
	...
	if (channel) {
	    ...
	    pthread_mutex_lock(&channel->mutex);
	    if (channel->status==CHANNEL_STATUS_UP) channel->substatus=CHANNEL_SUBSTATUS_S_EOF;
	    switch_channel_send_data(channel, "eof");
	    pthread_mutex_unlock(&channel->mutex);
...
static void receive_msg_channel_close(struct ssh_session_s *session, struct ssh_payload_s *payload)
{
	...
	if (channel) {
	    ...
	    pthread_mutex_lock(&channel->mutex);
	    channel->status=CHANNEL_STATUS_DOWN;
