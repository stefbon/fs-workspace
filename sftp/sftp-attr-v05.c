...
static void read_name_response_v05(struct sftp_subsystem_s *sftp, struct name_response_s *response, char **name, unsigned int *len, struct fuse_sftp_attr_s *fuse_attr)
{
    ...
    *name=(char *) pos; /* name without trailing zero */
    ...
    pos+=read_attributes_v05(sftp, pos, (unsigned int) (response->buff + response->size - pos), fuse_attr);
...
static void read_sftp_features_v05(struct sftp_subsystem_s *sftp)
{
	...
	logoutput("read_sftp_features_v05: sftp attr size supported");
	supported->fuse_attr_supported|=FUSE_SFTP_ATTR_SIZE;

    } else {

	logoutput("read_sftp_features_v05: sftp attr size not supported");
	...
	logoutput("read_sftp_features_v05: sftp attr permissions supported");

    } else {

	logoutput("read_sftp_features_v05: sftp attr permissions not supported");
	...
	logoutput("read_sftp_features_v05: sftp attr ownergroup supported");

    } else {

	logoutput("read_sftp_features_v05: sftp attr ownergroup not supported");
	...
	logoutput("read_sftp_features_v05: sftp attr atime supported");

    } else {

	logoutput("read_sftp_features_v05: sftp attr mtime not supported");
	...
	logoutput("read_sftp_features_v05: sftp attr mtime supported");
