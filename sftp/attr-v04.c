...
static unsigned int type_mapping[10]={0, S_IFREG, S_IFDIR, S_IFLNK, 0, 0};
...
static unsigned int read_sftp_attributes(struct sftp_subsystem_s *sftp, unsigned int valid, char *buffer, unsigned int size, struct sftp_attr_s *sftp_attr, struct fuse_sftp_attr_s *fuse_attr)
{
    ...
    vb[10]=(valid & SSH_FILEXFER_ATTR_EXTENDED) >> SSH_FILEXFER_INDEX_EXTENDED;
...
static unsigned int read_attributes_v04(struct sftp_subsystem_s *sftp, char *buffer, unsigned int size, struct fuse_sftp_attr_s *fuse_attr)
{
    struct sftp_attr_s sftp_attr;
    char *pos=buffer;
    unsigned int valid=0;

    logoutput_debug("read_attributes_v04");
...
static unsigned int write_attributes_v04(struct sftp_subsystem_s *sftp, char *buffer, unsigned int size, struct fuse_sftp_attr_s *fuse_attr)
{
...
static void read_sftp_features_v04(struct sftp_subsystem_s *sftp)
{
...
static unsigned int get_attribute_mask_v04(struct sftp_subsystem_s *sftp)
{
    return SSH_FILEXFER_STAT_VALUE;
}

static int get_attribute_info_v04(struct sftp_subsystem_s *sftp, unsigned int valid, const char *what)
{
...
static struct sftp_attr_ops_s attr_ops_v04 = {
    ...
    .get_attribute_info			= get_attribute_info_v04,
};

void use_sftp_attr_v04(struct sftp_subsystem_s *sftp_subsystem)
{
    sftp_subsystem->attr_ops=&attr_ops_v04;
}
