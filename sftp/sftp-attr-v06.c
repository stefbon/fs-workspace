...
struct sftp_attr_s {
    ...
    struct sftp_string_s		mime_type;
    uint32_t				link_count;
    struct sftp_string_s		untranslated_name;
...
static unsigned int read_attr_ownergroup(struct sftp_subsystem_s *sftp, char *buffer, unsigned int size, struct sftp_attr_s *attr, struct fuse_sftp_attr_s *fuse_attr)
{
    ...
    logoutput("read_attr_ownergroup: uid %i username %.*s gid %i  groupname %.*s", fuse_attr->user.uid, attr->owner.len, attr->owner.name, fuse_attr->group.gid, attr->group.len, attr->group.name);

    return 8 + attr->owner.len + attr->group.len;
}

static unsigned int read_attr_allocation_size(struct sftp_subsystem_s *sftp, char *buffer, unsigned int size, struct sftp_attr_s *attr, struct fuse_sftp_attr_s *fuse_attr)
{
    attr->allocation_size=get_uint64(buffer);
    return 8; /* 64 bits takes 8 bytes */
}
...
static unsigned int read_attr_changetime(struct sftp_subsystem_s *sftp, char *buffer, unsigned int size, struct sftp_attr_s *attr, struct fuse_sftp_attr_s *fuse_attr)
{
    attr->changetime=get_uint64(buffer);
    fuse_attr->ctime=attr->changetime;
    fuse_attr->valid[FUSE_SFTP_INDEX_CTIME]=1;
    fuse_attr->received|=FUSE_SFTP_ATTR_CTIME;
    return 8;
}

static unsigned int read_attr_changetime_n(struct sftp_subsystem_s *sftp, char *buffer, unsigned int size, struct sftp_attr_s *attr, struct fuse_sftp_attr_s *fuse_attr)
{
    attr->changetime_n=get_uint32(buffer);
    fuse_attr->ctime_n=attr->changetime_n;
    return 4;
}
...
static unsigned int read_attr_bits(struct sftp_subsystem_s *sftp, char *buffer, unsigned int size, struct sftp_attr_s *attr, struct fuse_sftp_attr_s *fuse_attr)
{
    ...
    attr->bits_valid=get_uint32(buffer+4);
    return 8;
}

static unsigned int read_attr_text_hint(struct sftp_subsystem_s *sftp, char *buffer, unsigned int size, struct sftp_attr_s *attr, struct fuse_sftp_attr_s *fuse_attr)
{
    attr->text_hint=(unsigned char) *buffer;
    return 1;
}

static unsigned int read_attr_mime_type(struct sftp_subsystem_s *sftp, char *buffer, unsigned int size, struct sftp_attr_s *attr, struct fuse_sftp_attr_s *fuse_attr)
{
    attr->mime_type.len=get_uint32(buffer);
    attr->mime_type.name=buffer+4;
    return attr->mime_type.len + 4;
}

static unsigned int read_attr_link_count(struct sftp_subsystem_s *sftp, char *buffer, unsigned int size, struct sftp_attr_s *attr, struct fuse_sftp_attr_s *fuse_attr)
{
    attr->link_count=get_uint32(buffer);
    return 4;
}

static unsigned int read_attr_untranslated_name(struct sftp_subsystem_s *sftp, char *buffer, unsigned int size, struct sftp_attr_s *attr, struct fuse_sftp_attr_s *fuse_attr)
{
    attr->untranslated_name.len=get_uint32(buffer);
    attr->untranslated_name.name=buffer+4;
    return attr->untranslated_name.len + 4;
}
...
static read_attr_cb read_attr_acb[][2] = {
	...
	{read_attr_zero, read_attr_text_hint},
	{read_attr_zero, read_attr_mime_type},
	{read_attr_zero, read_attr_link_count},
	{read_attr_zero, read_attr_untranslated_name},
...
static unsigned int read_sftp_attributes(struct sftp_subsystem_s *sftp, unsigned int valid, char *buffer, unsigned int size, struct sftp_attr_s *sftp_attr, struct fuse_sftp_attr_s *fuse_attr)
{
    ...
    logoutput("read_sftp_attributes");
    ...
    vb[11]=(valid & SSH_FILEXFER_ATTR_SUBSECOND_TIMES) >> SSH_FILEXFER_INDEX_SUBSECOND_TIMES;
    vb[12]=(valid & SSH_FILEXFER_ATTR_ACL) >> SSH_FILEXFER_INDEX_ACL;
    vb[13]=(valid & SSH_FILEXFER_ATTR_BITS) >> SSH_FILEXFER_INDEX_BITS;
    vb[14]=(valid & SSH_FILEXFER_ATTR_TEXT_HINT) >> SSH_FILEXFER_INDEX_TEXT_HINT;
    vb[15]=(valid & SSH_FILEXFER_ATTR_MIME_TYPE) >> SSH_FILEXFER_INDEX_MIME_TYPE;
    vb[16]=(valid & SSH_FILEXFER_ATTR_LINK_COUNT) >> SSH_FILEXFER_INDEX_LINK_COUNT;
    vb[17]=(valid & SSH_FILEXFER_ATTR_UNTRANSLATED_NAME) >> SSH_FILEXFER_INDEX_UNTRANSLATED_NAME;
    vb[18]=(valid & SSH_FILEXFER_ATTR_EXTENDED) >> SSH_FILEXFER_INDEX_EXTENDED;
    ...
    */
    ...
    pos += (* read_attr_acb[12][vb[12]])(sftp, pos, (unsigned int)(buffer + size - pos + 1), sftp_attr, fuse_attr);

    /*
	attrib_bits and attrib_bits_valid
    */

    pos += (* read_attr_acb[13][vb[13]])(sftp, pos, (unsigned int)(buffer + size - pos + 1), sftp_attr, fuse_attr);

    /*
	text hint
    */

    pos += (* read_attr_acb[14][vb[14]])(sftp, pos, (unsigned int)(buffer + size - pos + 1), sftp_attr, fuse_attr);

    /*
	mime type
    */

    pos += (* read_attr_acb[15][vb[15]])(sftp, pos, (unsigned int)(buffer + size - pos + 1), sftp_attr, fuse_attr);

    /*
	link count
    */

    pos += (* read_attr_acb[16][vb[16]])(sftp, pos, (unsigned int)(buffer + size - pos + 1), sftp_attr, fuse_attr);

    /*
	untranslated name
    */

    pos += (* read_attr_acb[17][vb[17]])(sftp, pos, (unsigned int)(buffer + size - pos + 1), sftp_attr, fuse_attr);

    /*
	extensions
    */

    pos += (* read_attr_acb[18][vb[18]])(sftp, pos, (unsigned int)(buffer + size - pos + 1), sftp_attr, fuse_attr);

    // logoutput("read_sftp_attributes: ready");

    return (unsigned int) (pos - buffer);

}

static unsigned int read_attributes_v06(struct sftp_subsystem_s *sftp, char *buffer, unsigned int size, struct fuse_sftp_attr_s *fuse_attr)
{
    struct sftp_attr_s sftp_attr;
    char *pos=buffer;
    unsigned int valid=0;

    logoutput("read_attributes_v06");
...
static unsigned int write_attr_accesstime(struct sftp_subsystem_s *sftp, char *pos, unsigned int size, struct fuse_sftp_attr_s *fuse_attr, unsigned int *valid)
{
    ...
    *valid|=SSH_FILEXFER_ATTR_ACCESSTIME;
    ...
    if (supported->version.v06.attribute_mask & SSH_FILEXFER_ATTR_SUBSECOND_TIMES) {
...
static unsigned int write_attr_changetime(struct sftp_subsystem_s *sftp, char *pos, unsigned int size, struct fuse_sftp_attr_s *fuse_attr, unsigned int *valid)
{
    struct sftp_supported_s *supported=&sftp->supported;

    store_uint64(pos, fuse_attr->ctime);
    *valid|=SSH_FILEXFER_ATTR_CTIME;

    if (supported->version.v06.attribute_mask & SSH_FILEXFER_ATTR_SUBSECOND_TIMES) {

	store_uint32(pos+8, fuse_attr->ctime_n);
	*valid|=SSH_FILEXFER_ATTR_SUBSECOND_TIMES;
...
static unsigned int write_attributes_v06(struct sftp_subsystem_s *sftp, char *buffer, unsigned int size, struct fuse_sftp_attr_s *fuse_attr)
{
	...
	len += fuse_attr->valid[FUSE_SFTP_INDEX_CTIME] * 12; /* change time (8+4)*/

	logoutput("write_attributes_v06: len %i", len);
	...
	pos+= (* write_attr_acb[5][fuse_attr->valid[FUSE_SFTP_INDEX_CTIME]]) (sftp, pos, (unsigned int)(buffer + size - pos + 1), fuse_attr, &valid);

	/* valid is set: write it at begin */

	store_uint32(buffer, valid);

	logoutput("write_attributes_v06: valid %i size %i", valid, (unsigned int) (pos - buffer));
...
/*
	...
	ATTRS				attr
    byte				end-of-list [optional]
*/

static void read_name_response_v06(struct sftp_subsystem_s *sftp, struct name_response_s *response, char **name, unsigned int *len)
{

    logoutput("read_name_response_v06: pos %i", (unsigned int)(response->pos - response->buff));
...
static unsigned int read_attr_response_v06(struct sftp_subsystem_s *sftp, struct name_response_s *response, struct fuse_sftp_attr_s *sftp_attr)
{
    char *keep=response->pos;

    memset(sftp_attr, 0, sizeof(struct fuse_sftp_attr_s));

    logoutput("read_attr_response_v06: pos %i", (unsigned int)(response->pos - response->buff));

    response->pos+=read_attributes_v06(sftp, response->pos, (unsigned int) (response->buff + response->size - response->pos), sftp_attr);
    response->count--;

    if (response->count==0 && (unsigned int) (response->buff + response->size - response->pos) > 0 ) {

	/* version 6 supports an optional end-of-file indicator */

	response->eof=(unsigned char) *response->pos;
	response->pos++;

    }

    logoutput("read_attr_response_v06: m %i p %i", sftp_attr->type, sftp_attr->permissions);
...
static void read_sftp_features_v06(struct sftp_subsystem_s *sftp)
{
    struct sftp_supported_s *supported=&sftp->supported;
	...
	logoutput("read_sftp_features_v06: sftp attr size supported");
	supported->fuse_attr_supported|=FUSE_SFTP_ATTR_SIZE;

    } else {

	logoutput("read_sftp_features_v06: sftp attr size not supported");
	...
	logoutput("read_sftp_features_v06: sftp attr permissions supported");

    } else {

	logoutput("read_sftp_features_v06: sftp attr permissions not supported");
	...
	logoutput("read_sftp_features_v06: sftp attr ownergroup supported");

    } else {

	logoutput("read_sftp_features_v06: sftp attr ownergroup not supported");
	...
	logoutput("read_sftp_features_v06: sftp attr atime supported");

    } else {

	logoutput("read_sftp_features_v06: sftp attr mtime not supported");
	...
	logoutput("read_sftp_features_v06: sftp attr mtime supported");
    ...
    if (attribute_mask & SSH_FILEXFER_ATTR_CTIME) {

	supported->fuse_attr_supported|=FUSE_SFTP_ATTR_CTIME;
	logoutput("read_sftp_features_v06: sftp attr ctime supported");

    } else {

	logoutput("read_sftp_features_v06: sftp attr ctime not supported");

    }

}

static unsigned int get_attribute_mask_v06(struct sftp_subsystem_s *sftp)
{
    struct sftp_supported_s *supported=&sftp->supported;
    logoutput("get_attribute_mask_v06: mask %i", supported->version.v06.attribute_mask);
    return (supported->version.v06.attribute_mask);
}

static struct sftp_attr_ops_s attr_ops_v06 = {
    .read_attributes			= read_attributes_v06,
    .write_attributes			= write_attributes_v06,
    .read_name_response			= read_name_response_v06,
    .read_attr_response			= read_attr_response_v06,
    .read_sftp_features			= read_sftp_features_v06,
    .get_attribute_mask			= get_attribute_mask_v06,
};

void use_sftp_attr_v06(struct sftp_subsystem_s *sftp_subsystem)
{
    sftp_subsystem->attr_ops=&attr_ops_v06;
}
